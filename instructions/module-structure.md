...
`modules/lifeforge--wishlist` (flat structure).

> Some modules have been migrated to Feature-Sliced Design (FSD). Their
> `client/src/` uses `entities/`, `features/`, `pages/`, `shared/` layers
> instead of the flat structure described below. Currently FSD:
> `books-library`, `calendar`, `movies`, `music`, `notes`, `passwords`,
> `todo-list`, `wallet`. See the oxlint `fsd-boundaries` plugin for the
