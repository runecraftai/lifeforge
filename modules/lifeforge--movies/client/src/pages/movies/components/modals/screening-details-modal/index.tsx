...
import type { TGVMovie } from '../tgvlist-modal/components/tgvmovie-item'
...
function ScreeningDetailsModal({
  ...
  function getSessionColor(session: (typeof filteredSessions)[number]) {
    const ratio =
      session.seatstotal > 0 ? session.seatsused / session.seatstotal : 0

    if (ratio >= 1) return TAILWIND_PALETTE.red[500]
    if (ratio > 0.5) return TAILWIND_PALETTE.amber[500]
    return TAILWIND_PALETTE.green[500]
  }
