...
function YoutubeDownloaderModal({ onClose }: { onClose: () => void }) {
  ...
  function getActionButtonProps() {
    if (isAPIKeyExistQuery.isLoading) return { loading: true }
    if (!isAPIKeyExistQuery.data) return undefined

    return {
      icon: 'mage:stars-c' as const,
  ...
  const hasValidVideoUrl = URL_REGEX.test(videoURL)
  const hasDownloadProgress = Boolean(downloadProgress)
