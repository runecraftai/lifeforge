...
function ItemList() {
  return (
    <WithQueryData contract={forgeAPI.items.list}>
      {items => (
        <ul>
          {items.map(item => (
...
function EventList({ startDate, endDate }: { startDate: string; endDate: string }) {
  return (
    <WithQueryData
      contract={forgeAPI.events.getByDateRange.input({
        start: startDate,
        end: endDate
      })}
    >
...
<WithQueryData
  ...
  errorFallback={<CustomError />} // Custom error fallback (default: ErrorScreen)
>
  {items => /* render items */}
</WithQueryData>
```
...
export function MyModuleProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <MyModuleContext value={{ items, isLoading }}>
      {children}
    </MyModuleContext>
  )
}
