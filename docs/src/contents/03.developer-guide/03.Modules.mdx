...
This directory contains the frontend code for the module. New modules use React with Tailwind CSS and shadcn-style UI. Pre-existing modules use `@lifeforge/ui` with vanilla-extract. A UI library called `lifeforge-ui` is provided in the `packages/` directory to maintain a consistent design language for modules that have not yet migrated.
...
interface ModuleConfig {
  ...
  // Optional normalized operation catalog
  contract?: Record<string, unknown>

  // Optional normalized operation catalog version
  contractVersion?: string
}
```

### Shared Host-Module Contract

`ModuleConfig` is the shared host-module boundary exported by `@lifeforge/configs`. Its `contract` field is an optional, framework-neutral operation catalog. The catalog is normalized data consumed by clients and other hosts; it must not contain Forge or Nest request objects, callbacks, transport URLs, credentials, or route handlers. A Forge serializer and a future Nest-native emitter must produce the same catalog shape.
...
| `noAuth` | Whether authentication is not required |
| `encrypted` | Whether the operation uses encrypted transport data |
| `isDownloadable` | Whether the response is a download |
| `media` | Media requirements, or `null` |
| `input` | Optional JSON Schema definitions for `query` and `body` |
| `output` | Response status map to JSON Schema, `true`, or `custom` |

The optional `contractVersion` field identifies the normalized catalog version, for example `1.0.0`. Existing manifests may omit it and remain valid; an omitted value means the catalog is unversioned. When present, the value is part of federation cache-key semantics. Use semantic versioning: increment the major version for breaking changes, the minor version for additive catalog changes, and the patch version for compatible corrections.
...
export default function ModuleProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ModuleContext value={{ state, setState }}>
      {children}
    </ModuleContext>
  )
}
...
2. **Pull to Code** – `pnpm forge db pull <module-name>`
3. **Commit** – Track your schema in version control
4. **Push (on install)** – `pnpm forge db push <module-name>` applies schema
...
```bash
# Build normal dist
pnpm forge modules build <module-name>

# Build Docker dist
pnpm forge modules build <module-name> --docker

# Build all modules
pnpm forge modules build
```
...
<Alert className="mt-6" type="tip">
  During development, the server runs source TypeScript directly (Pnpm executes TS natively). Builds are only needed for production deployment.
</Alert>
...
```bash
pnpm forge modules install lifeforge--calendar
```
...
```bash
pnpm forge modules install lifeforge--calendar --dev
```
...
4. Configure npm/pnpm with your token

### Publishing Your Module

```bash
pnpm forge modules publish <author>--<module-name>
```
...
```bash
$ pnpm forge modules publish myusername--my-module
  Version: 0.0.1 → 0.0.2
...
All module-related commands are under `pnpm forge modules`:
...
pnpm forge modules i lifeforge--calendar lifeforge--todo-list

# Uninstall
pnpm forge modules rm lifeforge--calendar

# Check for updates
pnpm forge modules up

# Build a specific module
pnpm forge modules build lifeforge--calendar

# Build all modules for Docker
pnpm forge modules build --docker
```
...
<Alert className="mt-6" type="tip">
  You can access the category management UI through the **Module Manager** in Settings. There, you can reorder categories, modify their translations, and perform AI auto-translate if you have the key configured in your key vault.
</Alert>
...
```bash
cd apps/myusername--my-module
pnpm add recharts zustand  # Module-specific packages
```
