...
```
    ...
    The `.typesMap()` method's available options depend on each field's TypeScript type. For instance, `number` fields support both `number` and `currency` types.
    ...
    In the `onSubmit` method, the `formData` parameter is guaranteed to be type-safe and aligned with the data structure defined in `defineForm`. This ensures confidence that submitted data is always valid and correctly formatted.
...
Consider the verbose approach required by libraries like `@tanstack/form`, where each field demands repetitive boilerplate:
...
Finally, call the `defineForm` function with the form data type as a generic parameter:
...
Import the `defineForm` function and the `FormModal` component from the LifeForge UI library. Then, create a component and call the `defineForm` function while passing the form data type as a generic parameter. chain it with a `.build()` call to get the final form props, then pass those props to the `FormModal` component being returned.
...
<Alert className="mt-6" type="note">
  The `FormModal` component should be the only thing being returned
  by the modal component.
</Alert>

### 3. Configuring the Form

In the argument object being passed into the `defineForm` function, you can specify various properties such as the form's title, icon, namespace, loading state, and submit button configuration. This allows you to customize the form's appearance and behavior right from the start. Detailed explanation of each property is listed below:
...
- <div className="text-lg!">
  ...
  These properties define the form's title and the icon displayed in the modal header, as shown in the figure below. The icon must be a valid icon identifier from Iconify in the form of `&lt;icon-set&gt;:&lt;icon-name&gt;`.
...
- <code className="text-lg!">onClose</code>
  ...
  This callback function is triggered when the close button is clicked. This should be the same `onClose` function being passed to the modal component, allowing the form to close itself when the user clicks the close button.

- <code className="text-lg!">submitButton</code>

  Can be either `"create"`, `"update"`, or an object containing the button's props if a custom button is needed. If either `"create"` or `"update"` is used, a predefined button will be rendered with the appropriate label and icon, as shown below:
...
Using the `.setupFields()` chain method, you can configure each field's properties such as label, icon, placeholder, validation rules, and more. This allows you to customize the form fields to suit your application's needs.
...
Sometimes you may want to show or hide certain fields in the form based on the values of other fields. This can be achieved using the `.conditionalFields()` chain method. This method accepts a mapping of field names to a function that returns a boolean indicating whether the field should be shown. The function has one parameter, which is the current form values.
...
Sometimes you will want to automatically focus a specific field when the form is opened. This can be achieved using the `.autoFocusField()` chain method.
...
```
  ...
  <Alert className="mt-6" type="note">
  The options for this method is determined by your definition in the `.typesMap()` chain method, and only fields that are auto-focusable will be available. IntelliSense will help you identify these fields.
...
Often times you will want to populate the form with existing data, such as when editing an existing record. This can be achieved using the `.initialData` chain method, which accepts an object that matches the form data type.
...
Finally, you can define the form submission logic using the `.onSubmit()` chain method. This method accepts an asynchronous function that receives the form data as its argument. You can perform any necessary actions with the data, such as sending it to ForgeAPI or processing it locally.
...
When defining configurations for each field, you can specify validation rules using the `validator` property. This property can be a function that returns either `true` if the value is valid or a string containing an error message if the value is invalid, like so:
...
LifeForge uses `zustand` for form state management. The `defineForm` function exposes a `formStateStore` object which is a Zustand store that you can use to get and set the form values. In addition, the `onClick` handler of the `actionButtonProps` also exposes the state getter and setter functions as its parameters.
...
Sometimes you may want to link two or more fields together, such as when one field's value depends on another field's value. This can be achieved easily using the `formStateStore`.

For example, the validation of the `confirmPassword` field could depend on the value of the `password` field.
