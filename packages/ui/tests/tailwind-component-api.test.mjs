...
describe('S2 - Component API and import verification', () => {
  describe('ModuleHeader', () => {
    it('C6: ModuleHeaderTailwind exists as a separate Tailwind-native export', () => {
      ...
      assert.ok(
        ...
        'ModuleHeader.tailwind.tsx should exist'
      )

      const source = readFileSync(componentPath, 'utf-8')
      assert.ok(
        source.includes('export function ModuleHeaderTailwind'),
        'Should export ModuleHeaderTailwind function'
      )
      assert.ok(
        source.includes('className='),
        'Should use className for Tailwind classes'
      )
      assert.ok(
        source.includes('bg-primary'),
        'Should use Tailwind color utility (bg-primary)'
      )
      assert.ok(
        source.includes('text-muted'),
        'Should use Tailwind color utility (text-muted)'
      )
      assert.ok(
        source.includes('rounded-lg'),
        'Should use Tailwind radius utility'
      )
  ...
  describe('import availability', () => {
    it('C7: tailwind.css exports @theme tokens as CSS custom properties', () => {
      ...
      assert.ok(
        tailwindCss.includes('@theme inline'),
    ...
    it('C7: tailwind.css is exported from package.json', () => {
      const pkg = JSON.parse(
        readFileSync(resolve(UI_ROOT, 'package.json'), 'utf-8')
      )
      assert.ok(
        pkg.exports['./tailwind.css'],
        'tailwind.css should be in exports'
      )
      assert.equal(
        pkg.exports['./tailwind.css'],
        './tailwind.css',
        'Export path should be correct'
      )
    })
  })
  ...
  describe('coexistence', () => {
    it('C8: tailwind.css and dist/index.css can both be imported without variable conflicts', () => {
      ...
      assert.ok(
        ...
        'dist/index.css must exist — run `pnpm build` in packages/ui before testing coexistence'
      )
